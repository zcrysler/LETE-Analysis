...
register_google(key = "AIzaSyAPKz-wAiGeaPy3JCexLdItxNcpknlwZDU")
...
```
## Study Area
Stations within the study region are presented in Figure \@ref(fig:2017StationMap), stations with red outlines had no detections, while coloured stations had at least one tag detected during the study period from 2017 to 2018. Triangles represent receivers that were active during 2017 *and* 2018, circles represent stations that were only active in 2017 (*Quintana* and *East Grand Terre*). The blue lines indicate the direction of antennas during the study period and span 15 miles from the receiver. All receivers maintained the same bearings throughout the study period with the exception of *Bolivar_Flats*, here the solid blue lines are the 2017 bearings, while the dashed blue lines are the 2018 bearings.

All Least Terns were tagged within close proximity of *Exxon Fields*, and for the purposes of this report stations considered within proximity to the breeding area are *Exxon Fields*, *East Grand Terre*, *Grand_Isle*, and *Wisner.* A close up of this area is presented in the inset of Figure \@ref(fig:2017StationMap).
...
bigMap <- ggplot(na.map, aes(long, lat)) +
  ...
  geom_segment(data = filter(df.antDeps, recvDeployID == 4616), # Create dashed lines for Bolivar 2018 antenna bearings
               aes(x = longitude, xend = longitude +
                     (sin(rad(antBearing)) *arr.sc), y = latitude, yend = latitude +
...
## Detection Overview
In 2017, only one individual (tag ID 136) was detected at stations outside the immediate tagging region, all other individuals and the bulk of the detections occurred at *Exxon Fields*, *East Grand Terre*, *Grand_Isle*, and *Wisner.* Figure \@ref(fig:2017DetectionMap) shows a map of all 2017 tagged bird detections, lines are coloured by tag ID. Figure \@ref(fig:2017LongitudePlots) displays the same data by plotting latitude vs. time for each tag, with points coloured by receiver site.
...
ggplot(na.map, aes(long, lat)) +
  ...
  guides(fill = FALSE,
         colour = guide_legend(override.aes = list(size = 2)))
ggsave("./images/detections2018.png")
...
For each station the total number of visits was calculated (Figure \@ref(fig:visitFrequency)), as expected *Exxon Fields* had by far the highest number of visits as it was at the nesting area. In 2017 and 2018 *Grand_Isle* had the next highest frequency of detections, this is not surprising as it is the closest station to the nesting area (see [table 2.1](#tab:distanceTable)). *East Grand Terre* and *Wisner* followed with lower numbers of site visits and are also near the nesting area. Note the lack of detections at *Wisner* in 2018, this is likely due to receiver malfunction rather than a true lack of detections. All other stations had negligible (<3) visits and are substantially farther away from the nesting grounds.
...
We can narrow down the areas visited beyond which site they were detected at by examining which antenna they were detected on during foraging. Figures \@ref(fig:detBearings2017) and \@ref(fig:detBearings2018) show the total number of detections for each antenna bearing at each site for 2017 and 2018 respectively, while figures \@ref(fig:CumulativeDetectionTimeMap17) and \@ref(fig:CumulativeDetectionTimeMap18) show the total number of birds and cumulative detection time of each antenna at each site for 2017 and 2018 respectively.
...
```{r detBearings2017, fig.cap = "Frequency of all 2017 foraging detections on each antenna. Increasingly darker blue bars indicate increased numbers of detections during the incubation period, orange lines indicate antenna bearings at each site. Note that scales differ by site.", echo = FALSE, message = FALSE, warning = FALSE, fig.height = 8}
# 2017
# East Grand Terre
...
```{r detBearings2018, fig.cap = "Frequency of all 2018 foraging detections on each antenna. Increasingly darker blue bars indicate increased numbers of detections during the incubation period, orange lines indicate antenna bearings at each site. Note that scales differ by site.", echo = FALSE, message = FALSE, warning = FALSE, fig.height = 8}
# 2018
# Bolivar_Flats
...
Both the number of detections and the cumulative detection time was highest at *Exxon Fields* in both years, followed by *Grand_Isle* and then *East Grand Terre*. All 21 individuals with foraging detections in 2017 were detected by both *Exxon Fields* and *Grand_Isle*, while in 2018 all 24 foraging individuals were detected at *Exxon Fields*, only 23 were detected at *Grand_Isle*. The cumulative detection time at *Grand_Isle* is also markedly shorter than at *Exxon Fiends* in 2018. In both years, smaller numbers of birds and shorter cumulative detection times occurred at *East Grand Terre*, and even fewer at *Wisner* (as noted previously, a lack of 2018 detections at *Wisner* is likely due to receiver malfunction rather than true lack of tags in the area).
...
```{r CumulativeDetectionCalcs, echo = FALSE, message = FALSE, warning = FALSE, results = "hide"}
...
# Get detection length at each station for each tag
lete_times <- foraging %>% mutate(date = as.Date(ts)) %>% 
  group_by(motusTagID, mfgID, tagDeployStart, tagDeployEnd, recvDeployName,port, runID) %>%
  summarize(det_len = round(as.numeric(difftime(max(ts), min(ts), units = "mins")), 3),
            date = min(date)) %>% as.data.frame()

lete_recvs <- for.dir %>% mutate(antBin = get_16wind(antBearing)) 
lete_recvs <- lete_recvs %>% left_join(compass, by = "antBin") %>% 
    # Approximate center of detection zones for each antenna and add to receiver data
  group_by_all() %>%
  do(detection_loc(.$recvLat, .$recvLon, .$antBinDeg, distkm = 2))

ant_avail <- ungroup(lete_recvs) %>% dplyr::select(recvDeployName, antBin, det_lon, det_lat) %>% distinct()

lete_time_ant_17 <- filter(lete_times, year(date) == 2017) %>% ungroup() %>%
  left_join(lete_recvs, by = c("recvDeployName")) %>%
  dplyr::select(motusTagID, recvDeployName, antBin, det_lat, det_lon, det_len) %>% 
  group_by(recvDeployName, antBin, det_lon, det_lat) %>%
  summarize(tot_det_len = sum(det_len)/60,
            n_ind = n_distinct(motusTagID)) %>%
  ungroup()# %>%
  complete(ant_avail, fill = list(tot_det_len = 0.005, n_ind = 0))

lete_time_ant_18 <- filter(lete_times, year(date) == 2018) %>% ungroup() %>%
left_join(lete_recvs, by = c("recvDeployName")) %>%
dplyr::select(motusTagID, recvDeployName, antBin, det_lat, det_lon, det_len) %>% 
group_by(recvDeployName, antBin, det_lon, det_lat) %>%
summarize(tot_det_len = sum(det_len)/60,
...
```{r CumulativeDetectionTimeMap17, fig.cap = "Cumulative 2017 detections of 21 foraging birds by each antenna at the colony site. Solid Blue lines indicate antenna bearings extending outward 10km from the receiver, numbers associated with each antenna indicate the number of tags detected, and the circles surrounding each number are coloured by cumulative detection time.", echo = FALSE, message = FALSE, warning = FALSE, fig.width = 5}

# Code for plot (CumulativeDetectionTimeMap17 and CumulativeDetectionTimeMap18) and associated dataframes courtesy of Adam Smith

gmap <-  get_map(location = c(lon = -90, lat = 29.2),
                       maptype = "terrain",
                       zoom = 10) 
ggmap(gmap) +
     xlab("") + ylab("") + theme_bw() +
  geom_segment(data = filter(df.antDeps, !(recvDeployID %in% c(5149, 4304, 4615, 4616))), # remove Quintana and Bolivar deployments outside detection range
               aes(x = longitude, xend = longitude +
                     (sin(rad(antBearing)) *.1), y = latitude, yend = latitude +
                     (cos(rad(antBearing)) *.1)), colour = "blue") +
  geom_segment(data = filter(df.antDeps, recvDeployID == 4616), # Create dashed lines for Bolivar 2018 antenna bearings
               aes(x = longitude, xend = longitude +
                     (sin(rad(antBearing)) *.1), y = latitude, yend = latitude +
                     (cos(rad(antBearing)) *.1)), colour = "blue", linetype = "dashed") +
geom_point(data = lete_time_ant_17, aes(x = det_lon, y = det_lat,
  ...
  scale_color_viridis_c("Cumulative detection time:", trans = "log2",
                       breaks = c(0.0166666, 1, 24, 120, 480),
                       labels = c("1min", "1h", "1d", "5d", "20d"),
                       limits = c(0.005, 480),
                       guide = guide_colorbar(order = 2, barwidth = grid::unit(0.4, "npc"), 
                                              title.vjust = 0.8,
                                              title.position = "left")) +
  # Receiving stations locations
geom_point(data = df.recvDepsCombined, aes(x = longitude, y = latitude, fill = incubate),
             shape = 21, colour = "black", size = 2.5) +
  # Number of individuals detected
  geom_text(data = lete_time_ant_17, aes(x = det_lon, y = det_lat+0.001, label = n_ind), 
  ...
  theme(legend.position = "top", 
   ...
   guides(fill = FALSE) +
  coord_fixed(xlim = c(-90.3, -89.7), ylim = c(29.45, 29))
ggsave("./images/AntennaBearingMap2017.png")

```

```{r CumulativeDetectionTimeMap18, fig.cap = "Cumulative 2018 detections of 24 foraging birds by each antenna at the colony site. Solid Blue lines indicate antenna bearings extending outward 10km from the receiver, numbers associated with each antenna indicate the number of tags detected, and the circles surrounding each number are coloured by cumulative detection time.", echo = FALSE, message = FALSE, warning = FALSE, fig.width = 5}

# Code for plot (CumulativeDetectionTimeMap17 and CumulativeDetectionTimeMap18) and associated dataframes courtesy of Adam Smith

gmap <-  get_map(location = c(lon = -90, lat = 29.2),
                       maptype = "terrain", # select maptype
                       zoom = 10) # zoom, must be a whole number
ggmap(gmap) +
     xlab("") + ylab("") + theme_bw() +
  geom_segment(data = filter(df.antDeps, !(recvDeployID %in% c(5149, 4304, 4615, 4616))), # remove Quintana and Bolivar deployments outside detection range
               aes(x = longitude, xend = longitude +
  ...
  geom_segment(data = filter(df.antDeps, recvDeployID == 4616), # Create dashed lines for Bolicvar 2018 antenna bearings
               aes(x = longitude, xend = longitude +
                     (sin(rad(antBearing)) *.1), y = latitude, yend = latitude +
                     (cos(rad(antBearing)) *.1)), colour = "blue", linetype = "dashed") +
geom_point(data = lete_time_ant_18, aes(x = det_lon, y = det_lat,
             size = tot_det_len, color = tot_det_len),
             shape = 21, fill = NA, stroke = 3) +
  scale_size(guide = "none", range = c(5,12)) + theme_bw() +
  scale_color_viridis_c("Cumulative detection time:", trans = "log2",
...
geom_point(data = df.recvDepsCombined, aes(x = longitude, y = latitude, fill = incubate),
             shape = 21, colour = "black", size = 2.5) +
  # Number of individuals detected
  geom_text(data = lete_time_ant_18, aes(x = det_lon, y = det_lat+0.001, label = n_ind), 
            color = "black", fontface = "bold", size = 3) +
  theme(legend.position = "top", 
        legend.box.just = "bottom",
        #axis.text=element_blank(),
   ...
   guides(fill = FALSE) +
  coord_fixed(xlim = c(-90.3, -89.7), ylim = c(29.45, 29))
ggsave("./images/AntennaBearingMap2018.png")
...
Only tags detected reliably during incubation at *Exxon Fields* were used for the following analysis, this included 5 tags from 2017, and 7 tags from 2018. In addition, only detections at *Exxon Fields* are used in this section.
...
ggsave("./images/SignalStrengthVSTime2017.png")
...
ggplot(filter(nestSite, year == 2018), aes(ts, sig, col = period)) + 
  ...
  facet_wrap(mfgID~., scales = "free_x", ncol = 2) + th + labs(x = NULL, y = "Signal Strength") +
  scale_color_discrete(labels = c("Day", "Night"), name = NULL)
ggsave("./images/SignalStrengthVSTime2018.png")
```


Colony site (*Exxon Fields*) detection plots in figures \@ref(fig:partitionFig17) and \@ref(fig:partitionFig18) show a clear difference between nocturnal and diurnal behaviours reflected in detection patterns. Nocturnal site visits were on average 30.41 (SE = 4.33) and 16.83 (SE = 1.88) minutes for 2017 and 2018 respectively, while detections during the day were 12.85  (SE = 0.81) and 10.73 (SE = 0.57) minutes for 2017 and 2018 respectively, mean detection length was significantly different (p < 0.001) for both years ([table 2.2](#tab:partitioningSummary)). Signal strength was also significantly different (p < 0.001) between nocturnal and diurnal detections in both years. Mean signal strength for each visit was 65.62 (SE = 1.12) and 59.01 (SE = 1.17) during the night compared 89.84 (SE = 0.73) and 73.58 (SE = 0.84) during the day for 2017 and 2018 respectively. This indicates important daily differences in Least Tern behaviour, during the night birds are more likely to be on the nest resulting in prolonged periods of detections with low signal strength, however during the day as the birds are travelling to and from the nest while foraging, they have shorter periods of nest attendance and higher signal strengths as birds in the air are more readily detected.
