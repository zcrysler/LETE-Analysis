...
By screening detections even further, and only examining detections that are below the signal strength threshold of 70 we can discern *nest* attendance. Once again detections were grouped into discrete "visits".
```{r, nestAttendanceResults, echo = FALSE}
#average length of visit
tmp1 <- filter(dayPeriods, period == "day") %>% group_by(year) %>% 
  summarize(meanLength = mean(visitLength)) %>% 
  as.data.frame()

# average number of daily visits
tmp <- filter(dayPeriods, period == "day") %>% group_by(year, date, motusTagID) %>% 
...
pander(tmp, booktabs = TRUE, caption = '(\\#tab:nestAttendance) Mean length and number of daylight nest attendance visits during 2017 and 2018', justify = 'left', style = "grid", split.cell = 25)
```
