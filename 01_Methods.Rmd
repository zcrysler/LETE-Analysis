...
```
## Time Partitioning
To determine time partitioning between foraging activities and nest site attendance during the incubation period, individuals that were consistently detected at the nesting site (*Exxon Fields*) were first selected. As previously shown, not all tags were consistently detected by Exxon Fields, detection patterns were visually inspected for all tags, and those that were not reliably detected at the nesting site were excluded. In total 5 and 7 tags were selected from 2017 and 2018 respectively. Periods of time during which it could not be confirmed that the bird was incubating were also excluded, for example, the nest of tag 28595 was predated sometime between May 18-21st, and the detection pattern clearly changes during this time, thus detections beyond May 18th were removed for this bird ([Figure 1.1](#fig:partitioningPlot1)). 

To discern behavioural patterns during the incubation period, detections were examined at two levels using the same technique. First all detections were examined to review general *site* attendance, or more specifically, the lack of site attendance indicating foraging. Second, only detections with a signal strength below 70 were examined as an indication of *nest* attendance, as periods of prolonged low signal strength generally indicate the Least Tern is on the nest. In both cases, only detections at *Exxon Fields* were examined for this subset of tags. 

For both levels, detection periods at *Exxon Fields* were split into discrete periods in the same way site visits were determined; consecutive detections at *Exxon Fields*, or runs of consecutive detections with less than 10 minutes between runs, were classified as one "visit". Furthermore, time of dawn and dusk was used to classify detections as occuring during the day or night.
...
tmp <- aov(data = filter(incPeriods, year == 2017), visitLength ~ period) # not significant, but incredibly small sample size
tmp <- aov(data = filter(incPeriods, year == 2018), visitLength ~ period) # not significant, but incredibly small sample size
summary(tmp) # significant difference 6.6e-11 ***

# Signal Strength vs Period ANOVA
tmp <- aov(data = filter(incPeriods, year == 2017), meanSig ~ period) # not significant, but incredibly small sample size
tmp <- aov(data = filter(incPeriods, year == 2018), meanSig ~ period) # not significant, but incredibly small sample size
summary(tmp) # significant difference 6.6e-11 ***
...
```
For *nest* attendance, visits spanning less than XX minutes were excluded as we would also expect decreasing signal strengths as the individual is flying away from the station to forage, as compared to prolonged periods of consistantly low signal strength indicative of nest attendance.
...
tmp1 <- dayPeriods %>% group_by(year, period) %>% 
  summarize(meanLength = mean(visitLength)) %>% 
  as.data.frame()
...
tmp2 <- tmp %>% group_by(year, period) %>% summarize(meanVisits = mean(totVisits)) %>% as.data.frame()
tmp <- left_join(tmp1, tmp2, by = c("year", "period"))
tmp <- rename(tmp, "Year" = "year", "Period" = "period", 
